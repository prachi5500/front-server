...
router.post('/verify-signup-otp', async (req, res) => {
  try {
    ...
    // 1. OTP memory store se check karo
    ...
    // 3. Password validate karo
    const passwordError = validatePassword(password);
    if (passwordError) {
      return res.status(400).json({ error: passwordError });
    }
    
    // 4. Naya user create karo (name aur phone OTP store se lo)
    const passwordHash = await bcrypt.hash(password, 10);
    const usersCount = await User.countDocuments();
    const role = usersCount === 0 ? 'admin' : 'user';
    ...
    const user = await User.create({
      email,
      passwordHash,
      name: otpData.name,    // ✅ OTP store se name
      phone: otpData.phone,  // ✅ OTP store se phone
      role,
      isVerified: true
    });
    
    // 5. OTP store clean karo
    otpStore.delete(email);
...
router.post("/request-reset", async (req, res) => {
  try {
    console.log("📧 Request-reset called with:", req.body);
    ...
    if (!user) {
      // Security ke liye same message bhejo
      return res.json({ message: "If the account exists, reset email sent." });
    }
    ...
    const frontendUrl = process.env.CORS_ORIGIN || 'https://front-server-six.vercel.app';
    ...
    const mailInfo = await transporter.sendMail({
      from: process.env.CONTACT_FROM,
      to: email,
      subject: "Password Reset - Business Cards",
      html: `
        <h2>Password Reset Request</h2>
        <p>Click the link below to reset your password:</p>
